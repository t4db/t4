...
// Register registers all Strata metrics on reg and returns the corresponding
// Gatherer. If reg is nil, a fresh isolated registry is created so that
// multiple Open() calls in the same process never collide.
func Register(reg prometheus.Registerer) prometheus.Gatherer {
	if reg == nil {
